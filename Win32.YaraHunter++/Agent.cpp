...
void Agent::ProcScanner(void(*callback)(HANDLE, t_udata*)) {
	t_udata			udata;
	uint8_t			*time;
	int				bwritten;
	HANDLE			snapshot;
...
void Agent::ScanRegion(HANDLE hProc, t_region_info *region, t_udata *udata) {
	...
	if (ReadProcessMemory(hProc, region->base, buffer, region->size, &b_read)
		...
		else yr_rules_scan_mem(this->yara->get_rules(), buffer, region->size, SCAN_FLAGS_PROCESS_MEMORY, (this->AgentCallback), udata, 0);
	}
	free(buffer);
}

void Agent::Callback_ScanPoc(HANDLE hProc, t_udata *udata) {
